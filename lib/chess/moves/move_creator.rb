...
module MoveCreator
  ...
  def castle?(from, to, board)
    ...
    return true if board.get_piece_at(from).is_a?(King) && castle_distance.include?(board.calculate_distance_vector(from, to))
    false
  end
  ...
  def en_passant?
    ...
    # For white
    if @from_piece.color == 'white' && @from[1] == '5' && en_passant_distance.include?(@board.calculate_distance_vector(@from, @to)) && @to_piece.nil?

      return true
    end

    # For black
    if @from_piece.color == 'black' && @from[1] == '4' && en_passant_distance.include?(@board.calculate_distance_vector(@from, @to)) && @to_piece.nil?
